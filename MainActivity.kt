...
fun CartScreen(cart: List<Product>) {
    ...
    if (goToPayment) {
        PaymentScreen(onSuccess = { success = true })
        return
    }
...
fun PaymentScreen(onSuccess: () -> Unit) {
    ...
    Column(modifier = Modifier.padding(20.dp)) {
        ...
        Text("💳 Select Payment Method", color = Color.Blue)
        ...
        when (method) {
            ...
            "UPI" -> {
                OutlinedTextField(
                    ...
                    label = { Text("Enter UPI ID (example@upi)") },
                    modifier = Modifier.fillMaxWidth()
                )
            }
            ...
            "QR" -> {
                ...
                Text("Tap below to simulate scan")
            }
        }
        ...
        Button(
            onClick = {
                if (method == "UPI" && upiId.isEmpty()) return@Button
                paymentDone = true
            },
            modifier = Modifier.fillMaxWidth()
        ) {
